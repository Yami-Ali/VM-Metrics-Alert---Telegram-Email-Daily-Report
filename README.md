...
│       --run                 │     │       windows.ps1  --run     │
│                             │     │                              │
│  • Checks all partitions    │     │  • Checks all drives C: D:…  │
...
> **Linux** monitors all real partitions independently — each partition (e.g. `sda1`, `pbxvg0-lvroot`) has its own cooldown timer.
...
└── state/
    ├── disk_tier_root_90         ← last alert: / at >=90%  (one file per partition per tier)
    ├── disk_tier_root_80         ← last alert: / at >=80%
    ├── disk_tier_root_70         ← last alert: / at >=70%
    ├── disk_tier_sda2_80         ← example: sda2 at >=80% (created as needed)
    ├── disk_tier_pbxvg0-lvvar_70 ← example: LVM volume at >=70% (created as needed)
...
```bash
docker exec -it n8n_db mysql -u mysql -pmysql n8n -e "SELECT ip, updated_at FROM vm_machines;"
```
...
> Linux shows one `💿 DISK USAGE` block per real partition instead of per-drive letter.

### Alert Email

Sent for all severity levels (CRITICAL, WARNING, NOTICE). The header color matches severity (red / orange / yellow). Body includes:
- VM name, IP, location, OS badge, status
- RAM usage bar + exact GB values
- Disk usage bar per drive (Windows) or per real partition (Linux)
...
| **Save VM to MySQL** | Upserts current metrics into the `vm_machines` table (keyed by IP) |
...
| **Read VMs from MySQL** | Reads all rows from `vm_machines` |
...
| **Clear VMs from MySQL** | Runs `DELETE FROM vm_machines` — resets for next day |
...
The `vm_machines` table is created automatically on first use. It stores one row per VM (keyed by IP, upserted on each report) and is cleared every morning after the daily report is sent.
...
| Disk monitoring | All real partitions (independent timers per partition) | All drives (C:, D:, E: ...) each independently |
